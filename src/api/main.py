...
async def lifespan(app: FastAPI):
    ...
    try:
        print("Buscando modelo de PRODUÇÃO no MLflow...")
        ...
        if not versions:
            ...
            print(f"Modelo de Produção encontrado no Run ID: {run_id} (version={model_registry_version}, algo={model_algorithm_loaded})")
    ...
    print("API desligada. Limpando recursos se necessário.")
...
def log_prediction(texto, predicao, probabilidade, tempo_ms):
...
@app.post("/predict")
def predict_sentiment(request: CustomerMessageRequest, background_tasks: BackgroundTasks):
    """
...
def submit_feedback(prediction_id: int, feedback: str, corrected_class: str = None):
    """
...
def get_low_confidence_predictions(threshold: float = 0.6, limit: int = 50):
    """
